...
float MQ2Sensor::MQGetGasPercentage(float rs_ro, int gas_id) {

  rs_ro = MQRead()/Ro;
  
  if (gas_id == GAS_CH4) {
     return MQGetPercentage(rs_ro,CH4Curve);
  } else if ( gas_id == GAS_CO ) {
     return MQGetPercentage(rs_ro,COCurve);
  } else if ( gas_id == GAS_SMOKE ) {
     return MQGetPercentage(rs_ro,SmokeCurve);
  } 

  return 0; 
}
int MQ2Sensor::MQGetPercentage(float rs_ro, float *pcurve) {
  rs_ro = MQRead()/Ro;
  return (pow(10,(((log(rs_ro)-pcurve[1])/pcurve[2]) + pcurve[0])));
}

String MQ2Sensor::read(){
  return String(MQGetGasPercentage((MQRead()/Ro), GAS_CH4));
}
